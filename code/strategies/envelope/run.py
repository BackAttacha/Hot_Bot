...
import cbpro  # Assuming you'll use the cbpro library

sys.path.append(os.path.join(os.path.dirname(__file__), '..', '..'))

# from utilities.bitget_futures import BitgetFutures # REMOVE THIS
from utilities.coinbase_advanced import CoinbaseAdvanced  # ADD THIS


# --- CONFIG ---
params = {
    'symbol': 'BTC-USDT',  # Coinbase uses a different symbol format
    'timeframe': '1h',
    'margin_mode': 'isolated',  # 'cross' -  Coinbase does not have this
    'balance_fraction': 1,
    'leverage': 1, # Coinbase does not have this
...
# bitget = BitgetFutures(api_setup) # REMOVE THIS
coinbase = CoinbaseAdvanced(api_setup) # ADD THIS
...
# --- CANCEL OPEN ORDERS ---
# orders = bitget.fetch_open_orders(params['symbol']) # REMOVE THIS
orders = coinbase.fetch_open_orders(params['symbol']) # ADD THIS
for order in orders:
    # bitget.cancel_order(order['id'], params['symbol']) # REMOVE THIS
    coinbase.cancel_order(order['id'], params['symbol']) # ADD THIS
# trigger_orders = bitget.fetch_open_trigger_orders(params['symbol']) # REMOVE THIS
trigger_orders = coinbase.fetch_open_trigger_orders(params['symbol']) # ADD THIS
long_orders_left = 0
short_orders_left = 0
for order in trigger_orders:
    if order['side'] == 'buy' and order['info']['tradeSide'] == 'open':
        ...
        short_orders_left += 1
    # bitget.cancel_trigger_order(order['id'], params['symbol']) # REMOVE THIS
    coinbase.cancel_trigger_order(order['id'], params['symbol']) # ADD THIS
...
# data = bitget.fetch_recent_ohlcv(params['symbol'], params['timeframe'], 100).iloc[:-1] # REMOVE THIS
data = coinbase.fetch_recent_ohlcv(params['symbol'], params['timeframe'], 100).iloc[:-1] # ADD THIS
...
# closed_orders = bitget.fetch_closed_trigger_orders(params['symbol']) # REMOVE THIS
closed_orders = coinbase.fetch_closed_trigger_orders(params['symbol']) # ADD THIS
...
# positions = bitget.fetch_open_positions(params['symbol']) # REMOVE THIS
positions = coinbase.fetch_open_positions(params['symbol']) # ADD THIS
if positions:
    sorted_positions = sorted(positions, key=lambda x: x['timestamp'], reverse=True)
    latest_position = sorted_positions[0]
    for pos in sorted_positions[1:]:
        # bitget.flash_close_position(pos['symbol'], side=pos['side']) # REMOVE THIS
        coinbase.flash_close_position(pos['symbol'], side=pos['side']) # ADD THIS
        print(f"{datetime.now().strftime('%H:%M:%S')}: double position case, closing the {pos['side']}.")


# --- CHECKS IF A POSITION IS OPEN ---
# position = bitget.fetch_open_positions(params['symbol']) # REMOVE THIS
position = coinbase.fetch_open_positions(params['symbol']) # ADD THIS
...
if 'price_jump_pct' in params and open_position:
    if position['side'] == 'long':
        if data['close'].iloc[-1] < float(position['info']['openPriceAvg']) * (1 - params['price_jump_pct']):
            # bitget.flash_close_position(params['symbol']) # REMOVE THIS
            coinbase.flash_close_position(params['symbol']) # ADD THIS
            update_tracker_file(tracker_file, {
                "last_side": "long",
...
if not open_position:
    # bitget.set_margin_mode(params['symbol'], margin_mode=params['margin_mode']) # REMOVE THIS
    # bitget.set_leverage(params['symbol'], margin_mode=params['margin_mode'], leverage=params['leverage']) # REMOVE THIS
    pass # Coinbase does not have this
...
if open_position:
    ...
    # exit
    # bitget.place_trigger_market_order( # REMOVE THIS
    coinbase.place_trigger_market_order( # ADD THIS
    ...
    # sl
    # sl_order = bitget.place_trigger_market_order( # REMOVE THIS
    sl_order = coinbase.place_trigger_market_order( # ADD THIS
        symbol=params['symbol'],
        side=close_side,
        amount=amount,
...
# balance = params['balance_fraction'] * params['leverage'] * bitget.fetch_balance()['USDT']['total'] # REMOVE THIS
balance = params['balance_fraction'] * params['leverage'] * coinbase.fetch_balance()['USDT']['total'] # ADD THIS
...
if long_ok:
    for i in range_longs:
        ...
        # min_amount = bitget.fetch_min_amount_tradable(params['symbol']) # REMOVE THIS
        min_amount = coinbase.fetch_min_amount_tradable(params['symbol']) # ADD THIS
        if amount >= min_amount:
            # entry
            # bitget.place_trigger_limit_order( # REMOVE THIS
            coinbase.place_trigger_limit_order( # ADD THIS
                symbol=params['symbol'],
                side='buy',
                amount=amount,
